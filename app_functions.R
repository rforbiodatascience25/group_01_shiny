...
gene_dna <- function(length, base_probs = c(0.25, 0.25, 0.25, 0.25)){
  ...
  if ( abs(sum(base_probs) - 1) > 1e-10){
    stop("The base probabilities must all sum up to 1, currently at: ", sum(base_probs))
  }
  dna_vector <- sample(
    x = c("A", "T", "C", "G"),
    size = length,
...
plot_dist <- function(df){
  p1 <- ggplot(df, aes(x = dna_vec, y = Freq, fill = dna_vec)) +
    ...
    labs(title = "Nucleotide Frequency Histogram",
         x = "Nucleotide Base",
         y = "Frequency")
